...
class SectionsViewModel: BaseViewModel {
    // MARK: - RxSwif Variables
    let isSuccess: PublishSubject<SectionsModel> =
        PublishSubject()
    ...
    var message = ""
    // MARK: - init
    public init (_ repo: SectionsRepository = SectionsRepository()) {
        repository = repo
    ...
    func getSectionsData(idd: Int) {
        ...
        repository.getSectionsData(idd: idd) { [weak self] (result) in
            ...
            case .success(let data):
                if let data = data as? SectionsModel {
