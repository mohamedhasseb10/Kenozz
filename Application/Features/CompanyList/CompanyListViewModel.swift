...
class CompanyListViewModel: BaseViewModel {
    ...
    let isSuccess: PublishSubject<CompanyListModel> =
        PublishSubject()
    let isLoading: PublishSubject<Bool> = PublishSubject()
    let isError: PublishSubject<ErrorMessage> = PublishSubject()
    let disposeBag: DisposeBag = DisposeBag()
    // MARK: - ViewModel Variables
    let repository: CompanyListRepository
    var message = ""
    // MARK: - init
    public init (_ repo: CompanyListRepository = CompanyListRepository()) {
        repository = repo
        isSuccess.disposed(by: disposeBag)
        configureDisposeBag()
    }
    func getCompanyListData(idd: Int) {
        self.isLoading.onNext(true)
        repository.getCompanyListData(idd: idd) { [weak self] (result) in
            ...
            case .success(let data):
                if let data = data as? CompanyListModel {
